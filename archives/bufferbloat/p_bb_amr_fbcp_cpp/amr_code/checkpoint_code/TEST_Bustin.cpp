...
Amr Radwan Optimal Control of AQM, July, 2015
----------------------------------------------------------------------------*/
...
double mu_bastin = 50.0;
double R_bastin = 0.2;
double w_bastin= 60.0;
double t_burst = 1.0;
double a_bastin=1.0;
// double t_0 = 0.0;			/*  initial time */
// double t_f = 2.0;			/*  terminal time */
...
{double Cont;
	    ...
	    if(0.0<=t<5.00500501e-02)
              {
		Cont=w_bastin;
	      }
              else if(t_burst<t<=2.0)
             {
		Cont=0.0;
	      }
	      else
		 {
		    Cont=mu_bastin*(1.0-sqrt(a_bastin/(R_bastin*mu_bastin)));
	         }
	         
	return Cont;
}

void func(double X[1],double t, double F[1])
{
	F[0] = -(mu_bastin*X[0]/(a_bastin+X[0])) + func_U(t);
}

void func_lambda(double L[1], double X[1], double F[1])
{
	F[0] = -1.0 + L[0]*(mu_bastin*a_bastin)/((a_bastin+X[0])*(a_bastin+X[0]));
}

void func_adj(double BF[1], double X[1], double BX[1])
{
	BX[0] = 1.0 - BF[0]*(mu_bastin*a_bastin)/((a_bastin+X[0])*(a_bastin+X[0]));
}



void advance(double F[1],double F_H[1],double t,double h)
{
	double k0[1],k1[1],G[1];
...
void store(double F_H[1], double **F_C,double t,int i)
{
	F_C[0][i] = F_H[0]; 
	F_C[1][i] = t;	
}

void restore(double F_H[1], double **F_C,double *t,int i)
{
	F_H[0] = F_C[0][i]; 
	*t = F_C[1][i];
}
...
void adjoint(double L_H[1],double F_H[1],double L[1],double t,double h)
{
	double k0[1],k1[1],G[1],BH[1],Bk0[1],Bk1[1],BG[1];
...
int main(int argc, char *argv[])
{
	...
	double F[1],L[1];
	double t,h,F_H[1],L_H[1],F_final[1];
	double **F_Check;

	cout << "**************************************************************************" << endl;
	cout << "*        Solution of the optimal control problem of AQM                  *" << endl;
	cout << "*                       by Amr Radwan                                    *" << endl;
	cout << "*                                                                        *" << endl;
	cout << "*           J = x(t) + R(w(t)-u(t)) -> min                               *" << endl;
	cout << "*           s.t.   dx/dt = u(t) - mu*x / (a+x),            x(0)=0        *" << endl;
	cout << "*                                                                        *" << endl;
	cout << "*                  the adjoint equation fulfill                          *" << endl;
	cout << "*                                                                        *" << endl;
	cout << "*       dx_bar/dt = -1.0 + x_bar(t) * (a*mu)/(a+x)**2      x_bar(2)=0    *" << endl;
	cout << "*                                                                        *" << endl;
	...
	double ratio_run_time;
        ratio_run_time=r->expense(steps,snaps);
	...
	cout << "\n\n";
	cout << "\n ratio_run_time = " << ratio_run_time<<endl;
	cout << "\n\n";
	cout << "**************************************************************************" << endl;
        cout << "*                                                                        *" << endl;
	cout << "*             run the code for different SNAPS then                      *" << endl;
	cout << "*             draw a figure SNAPS & ratio_run_time                       *" << endl;
	cout << "*                                                                        *" << endl;
	cout << "*                         Hint:                                          *" << endl;
	cout << "*   The ratio is between the run-time needed by reversal schedule        *" << endl;
	cout << "*         and run-time needed by straight forward approach               *" << endl;
	cout << "*                                                                        *" << endl;
	cout << "**************************************************************************" << endl;
}
