...
class PlotFormatter(Formatter):
	...
	#LINH MODIFICATION
        axis.set_xlabel('Time',fontsize=20) # add fontsize=20, for only timeseries plot, not affect to cdf plot
	axis.set_ylabel('',fontsize=20) #add more
	for xtick in axis.xaxis.get_major_ticks():
	    xtick.label.set_fontsize(20)
	for ytick in axis.yaxis.get_major_ticks():
	    ytick.label.set_fontsize(20)
	#finish of adding more
    ...
    def _annotate_plot(self, skip_title=False):
        ...
        if self.settings.PRINT_TITLE:
            ...
            if 'description' in self.config and self.settings.TITLE and not skip_title:
                y=1.00
            titles.append(self.figure.suptitle(plot_title, fontsize=20, y=y)) #LINH MODIFICATION (origin: fontsize=14)
        ...
        if self.settings.ANNOTATE:
            ...
            titles.append(self.figure.text(0.5, 0.0, annotation_string,
                                            ...
                                            fontsize=10)) 	#LINH MODIFICATION (origin: fontsize=8)
        return titles
    ...
    def _do_legend(self, config, postfix=""):
        ...
        l = axes[0].legend(handles, labels,
                                ...
                                prop={'size':'16'}, #LINH MODIFICATION origin 'size': 'small' 
                                **kwargs)
	t = l.get_title() #LINH add more
	t.set_fontsize(16) #LINH add more
