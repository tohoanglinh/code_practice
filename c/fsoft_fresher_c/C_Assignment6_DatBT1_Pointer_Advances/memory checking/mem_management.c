...
static uint32_t mm_check_buf_valid(MEM_ENTRY_HEAD_PTR buf_head);
static uint32_t mm_dump_buf_invalid(MEM_ENTRY_HEAD_PTR buf_head);
...
(
    ...
    if (isZero) {
        _mm_internal_zero(((uint8_t *) buf_head) + sizeof(MEM_ENTRY_HEAD), request_size);
    }/* Endif */
    ...
    */
    ...
    if (NULL != g_mem_allocation_table.buf_ptr[idx]) {
        printf("\n\rmemory allocation table is exceed (%d buf(s))",
    ...
    buf_head->allocated_add = (uint32_t)(((uint8_t *) buf_head) + sizeof(MEM_ENTRY_HEAD));
...
(
    ...
    printf("\n\r");
    printf("\n\r ******************* ERROR when free a buffer(0x%0x) with head(0x%0x) which is allocated at file %s:%d *******************",
...
(
    ...
    if (ret != MQX_OK) {
        ...
        if (idx == MEM_MAX_SAVE_BUF_PTR) {
            ...
            printf("\n\r ****Trying to free Buffer, which not resist in Allocation Table *** ");
            mm_dump_buf_invalid(buf_head);
...
void mm_display() {
    ...
    printf("\n\rTotal memory allocated: %lu byte(s)",
           g_mem_allocation_table.totalAllocSize);
    printf("\n\rTotal memory free:      %lu byte(s)",
           g_mem_allocation_table.totalFreedSize);
    printf("\n\rTotal memory unfree:    %lu byte(s)",
           g_mem_allocation_table.totalAllocSize - g_mem_allocation_table.totalFreedSize);
    printf("\n\rTotal memory allocated from *New* operator: %lu byte(s)",
           g_mem_allocation_table.totalNewSize);
    printf("\n\rTotal memory free from *Delete* operator: %lu byte(s)",
...
void mm_display_unfree() {
    ...
    for (i = 0; i < MEM_MAX_SAVE_BUF_PTR; i++) {
        ...
        if (NULL != buf_head ) {
            ...
            /*          uint32_t j;           */
            printf("\n\r\n\rBuffer which is allocated at:");
            printf("\n\r    File           %s", buf_head->file_name);
            printf("\n\r    Line           %d", buf_head->file_loc);
            printf("\n\r    Pointer        0x%X", buf_head->allocated_add);
            printf("\n\r    Requested size %d", buf_head->requested_size);
        }/* Endif */
    }
    /* Endfor */
}/* Endbody */
